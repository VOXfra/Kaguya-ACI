...
if(typeof v115CreativeItems==='function'){
 ...
 v115CreativeItems=function(cfg){
  return (v120CreativeItemsBase(cfg)||[]).filter(p=>{
   if(!p)return false;
   if(p.v117GenericBinder||p.v117StorageBinder||p.v117CanonicalBooster||p.contentKind==='binder')return true;
...
setTimeout(()=>{
 try{v120NormalizeBoosterLots();v120RepairPendingOpening();save();if($('#shop')?.classList.contains('active'))renderProducts();if($('#binder')?.classList.contains('active'))renderBinder()}
